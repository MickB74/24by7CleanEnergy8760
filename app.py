...
if st.session_state.analysis_complete and st.session_state.portfolio_data:
    ...
    with m1:
        st.metric(
            ...
            help="**Carbon Free Energy Score (Volumetric):** The percentage of your total electricity consumption that is matched with carbon-free energy generation on an hourly basis. Calculated as (Total Matched MWh / Total Load MWh)"
        )
    ...
    with m2:
        st.metric(
            ...
            help="**Loss of Green Hours:** The percentage of hours in the year where your renewable generation is less than your load, requiring grid imports"
        )
    ...
    with m3:
        st.metric(
            ...
            help="**Annual Renewable Match:** The ratio of your total annual renewable generation to your total annual electricity consumption. This can be >100% even if CFE % is lower, due to overgeneration during peak hours"
        )
    ...
    with m4:
        ...
        st.metric(
            ...
            help="**Hours Fully Matched:** The number of hours in the year (out of 8760) where your renewable generation fully meets or exceeds your load"
        )
    ...
    with m_gen:
        st.metric(
            ...
            help="**Total Generation:** The total amount of renewable energy generated by your portfolio assets over the entire year"
        )
    ...
    with m_load:
        st.metric(
            ...
            help="**Total Load:** The total amount of electricity consumed by your building portfolio over the entire year"
        )
    ...
    with m5:
        st.metric(
            ...
            help="**Grid Consumption:** The total amount of electricity you still need to buy from the grid because your renewable generation was insufficient during those specific hours"
        )
    ...
    with m6:
        st.metric(
            ...
            help="**Overgeneration:** The total amount of renewable energy you generated that exceeded your load during specific hours. This energy is typically sold back to the grid or curtailed"
        )
    ...
    with f3:
        st.metric(
            ...
            help="**Net Financial Position:** Total REC Revenue minus Total REC Cost. Positive values indicate a net profit from REC sales, while negative values indicate a net cost"
        )
    ...
    with st.expander("Show REC Valuation Logic"):
        if use_rec_scaling:
            # Calculate dynamic prices for display based on Base Price
            # Multipliers: Critical=2.0, Winter Morning=1.4, Evening=1.2, Shoulder=1.0, Mid-Day=0.75, Abundant=0.45
            
            st.markdown(f"""
            **REC Pricing Categories (based on grid scarcity):**
            
            *   **Critical Scarcity (${base_rec_price * 2.0:.2f}/MWh):** Winter Evening Peak (Dec-Feb, 18:00-20:00). Highest value.
            *   **Evening Peak (${base_rec_price * 1.2:.2f}/MWh):** Typical Evening Peak (17:00-21:00). High value.
            *   **Winter Morning Scarcity (${base_rec_price * 1.4:.2f}/MWh):** Winter Mornings (Dec-Feb, 06:00-09:00).
            *   **Shoulder Daylight (${base_rec_price * 1.0:.2f}/MWh):** Morning/Afternoon transition (07:00-10:00 & 15:00-18:00).
            *   **Typical Mid-Day (${base_rec_price * 0.75:.2f}/MWh):** Nov-Feb Mid-day (10:00-15:00).
            *   **Super-Abundant Mid-Day (${base_rec_price * 0.45:.2f}/MWh):** Mar-Oct Mid-day (10:00-15:00). Lowest value due to solar abundance.
            """)
        else:
            st.markdown(f"""
            **Flat Pricing Logic:**
